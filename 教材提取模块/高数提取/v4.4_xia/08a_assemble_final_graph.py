...
def build_section_groups(nodes: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    section_grouped: dict[str, list[dict[str, Any]]] = {}
    subsection_grouped: dict[str, list[dict[str, Any]]] = {}
    for node in nodes:
        textbook_id = str(node.get("textbook_id") or "").strip()
        ...
        group_key = "|".join([textbook_id, chapter, section]) if section else section_node_id
        if group_key:
            section_grouped.setdefault(group_key, []).append(node)
        if section_node_id:
            subsection_grouped.setdefault(section_node_id, []).append(node)

    groups: list[dict[str, Any]] = []
    edges: list[dict[str, Any]] = []
    section_group_ids: dict[str, str] = {}

    for group_key, members in sorted(section_grouped.items()):
        first = members[0]
        group_id = stable_id(f"{first.get('textbook_id', '')}:group", [group_key, "SectionGroup"])
        section_group_ids[group_key] = group_id
        section_node_ids = sorted({
            str(member.get("section_node_id") or "").strip()
            for member in members
            if str(member.get("section_node_id") or "").strip()
        })
        subsections = sorted({
            str(member.get("subsection") or "").strip()
            for member in members
            if str(member.get("subsection") or "").strip()
        })
        group = ensure_source_code(
            {
                "group_id": group_id,
                "name": str(first.get("section") or first.get("subsection") or group_key),
                "type": "KnowledgeGroup",
                ...
                "section_node_id": section_node_ids[0] if section_node_ids else "",
                "section_node_ids": section_node_ids,
                "subsections": subsections,
                "subsection_count": len(subsections),
                "source_scope": first.get("source_scope", ""),
                "member_count": len(section_node_ids),
                "core_member_count": len(members),
                "creation_policy": "auto_by_textbook_chapter_section",
    ...
    for section_node_id, members in sorted(subsection_grouped.items()):
        first = members[0]
        textbook_id = str(first.get("textbook_id") or "").strip()
        chapter = str(first.get("chapter") or "").strip()
        section = str(first.get("section") or "").strip()
        section_group_key = "|".join([textbook_id, chapter, section]) if section else section_node_id
        section_group_id = section_group_ids.get(section_group_key, "")
        group_id = stable_id(f"{first.get('textbook_id', '')}:group", [section_node_id, "SubsectionGroup"])
        group_name = str(first.get("subsection") or first.get("section") or section_node_id)
        group = ensure_source_code(
            {
                "group_id": group_id,
                "name": group_name,
                "type": "KnowledgeGroup",
                "group_type": "SubsectionGroup",
                "kg_layer": "knowledge_group",
                "textbook_id": first.get("textbook_id", ""),
                "textbook_name": first.get("textbook_name", ""),
                "chapter": first.get("chapter", ""),
                ...
                "section_group_key": section_group_key,
                "parent_group_id": section_group_id,
                "parent_group_type": "SectionGroup" if section_group_id else "",
        ...
        if section_group_id:
            edges.append(
                ensure_source_code(
                    {
                        "edge_id": stable_id(f"{first.get('textbook_id', '')}:groupedge", [section_group_id, group_id, "HAS_MEMBER"]),
                        "type": "HAS_MEMBER",
                        "kg_layer": "knowledge_group",
                        "source_group_id": section_group_id,
                        "source_node_id": section_group_id,
                        "source_name": str(first.get("section") or section_group_key),
                        "source_type": "KnowledgeGroup",
                        "target_group_id": group_id,
                        "target_node_id": group_id,
                        "target_name": group["name"],
                        "target_type": "KnowledgeGroup",
                        "textbook_id": first.get("textbook_id", ""),
                        "textbook_name": first.get("textbook_name", ""),
                        "chapter": first.get("chapter", ""),
                        "section": first.get("section", ""),
