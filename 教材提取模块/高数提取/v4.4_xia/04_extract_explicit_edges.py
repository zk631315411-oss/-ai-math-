"""
v4.4 Step 4A: extract explicit relation candidates between audited nodes.
...
Step 4A keeps LLM edge extraction clean: it does not rewrite edge directions or
auto-generate semantic attribution edges. Suspect edges are left as-is for
prompt improvement or later review.
"""
...
DEFAULT_REVIEW = DEFAULT_OUTPUT_DIR / "edge_pre_audit_review_queue.jsonl"
...
PROPERTY_TARGET_TYPES = {"Concept", "Theorem", "Formula"}
PROPERTY_HINT_RE = re.compile(r"(性质|定理|准则|公式|法则|推论|判定|结论|关系|影响|展开)")
CONDITIONAL_OR_EXAMPLE_EVIDENCE_RE = re.compile(r"(有些|某些|部分|并非所有|不一定|例如|这两个|这些|也可以|也可|可看成|可看作|可表示|可以表示|当.*时|若.*则|如果.*则)")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--review", type=Path, default=DEFAULT_REVIEW, help="Pre-audit queue from Step 4A validation; Step 4E writes the final edge_review_queue.jsonl.")
...
def build_node_pool(section: dict[str, Any], nodes: list[dict[str, Any]], max_nodes: int) -> list[dict[str, Any]]:
    visible = [node for node in nodes if node_visible_for_section(node, section)]
    ...
    for node in ordered:
        ...
        seen.add(node_id)
        deduped.append(node)
        if len(deduped) >= max_nodes and not node_current_section(node, section):
            break
    return deduped
...
def looks_like_defined_term(name: str, text: str) -> bool:
    compact = compact_text(text)
    clean_name = compact_text(name)
    if not clean_name:
        return False
    return any(f"{verb}{clean_name}" in compact for verb in ("称为", "叫做", "叫作", "记为", "记作"))
...
def validate_edge(edge: dict[str, Any], section_text: str, node_pool_ids: set[str]) -> list[str]:
    ...
    compact_evidence = compact_text(combined_evidence)
    source_name = str(edge.get("source_name") or "")
    target_name = str(edge.get("target_name") or "")
    compact_source = compact_text(source_name)
    compact_target = compact_text(target_name)
    if combined_evidence:
        if compact_source and compact_source not in compact_evidence:
            warnings.append("source_name_not_in_evidence_requires_review")
        if compact_target and compact_target not in compact_evidence:
            warnings.append("target_name_not_in_evidence_requires_review")
    if edge_type == "SUPERIOR":
        if looks_like_defined_term(source_name, combined_evidence):
            warnings.append("superior_source_defined_in_evidence_requires_review")
        if "分支" in target_name:
            warnings.append("superior_branch_target_requires_review")
        if CONDITIONAL_OR_EXAMPLE_EVIDENCE_RE.search(combined_evidence):
            warnings.append("superior_conditional_or_example_evidence_requires_review")
    if edge_type == "HAS_PROPERTY":
        if source_type in {"Theorem", "Formula", "Method"} and target_type == "Concept":
            warnings.append(f"has_property_possible_reverse:{source_type}->{target_type}")
        if target_type == "Method":
            warnings.append("has_property_method_target_requires_review")
        if target_type == "Concept" and not PROPERTY_HINT_RE.search(target_name):
            warnings.append("has_property_plain_concept_target_requires_review")
    ...
    if edge.get("source_review_status") != "auto_accept" or edge.get("target_review_status") != "auto_accept":
        warnings.append("edge_touches_review_node")
        if not edge.get("review_reason"):
            edge["review_reason"] = "边连接了 Step 3E 待审节点，需随端点节点一起进入 Step 7 复核。"
    ...
    if edge_type == "EQUATIVE":
        warnings.append("equative_weak_relation")
    if edge_type == "DERIVES":
        warnings.append("derives_direction_note")
    return warnings


def decide_review_status(warnings: list[str]) -> str:
    informational_warnings = {
        "equative_weak_relation",
        "derives_direction_note",
    }
    actionable_warnings = [warning for warning in warnings if warning not in informational_warnings]
...
def process_raw_edge(
    ...
    if hard_warnings:
        edge["pre_audit_hard_warnings"] = hard_warnings
...
def main() -> None:
    ...
    model = args.model or llm_config.get("edge_model") or llm_config.get("high_risk_model", "GPT-5.5")
