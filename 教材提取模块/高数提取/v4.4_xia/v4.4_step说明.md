...
- Step 8A 根据 Step 7 的 approved 包和 `merge_plans.jsonl` 组装最终图谱，执行合并计划，并自动生成确定性 `KnowledgeGroup`，包括按教材自然节生成的 `SectionGroup`、按叶子小节生成的 `SubsectionGroup`，以及按规则案例所属节点生成的 `RuleGroup`。这些组不由 LLM 自由发明。
...
- 删除合并后产生的自环边和重复边。
- 生成 `SectionGroup`、`SubsectionGroup` 和 `RuleGroup`。
...
v4.4 在 Step 8A 额外自动生成知识组：

- `SectionGroup`：按“教材 ID + 章 + 教材自然节”自动生成，一个教材自然节一个组。该组通过 `HAS_MEMBER` 指向该节下的 `SubsectionGroup`，用于应用端按教材章节导航和召回。
- `SubsectionGroup`：按 `section_node_id` 自动生成，一个叶子小节一个组。该组通过 `HAS_MEMBER` 指向本叶子小节通过审核的核心知识点，用于保留精细来源和小节内聚合。
...
- `SectionGroup`：由教材自然节自动生成，组名采用教材节名，例如 `10.1 二重积分`。它只承担大节导航和应用召回，不直接表示数学语义关系。
- `SubsectionGroup`：由叶子小节自动生成，组名采用叶子小节名，来源编码来自 `section_node_id`。它用于保留精细来源，并通过 `HAS_MEMBER` 指向该叶子小节内的核心知识点。
