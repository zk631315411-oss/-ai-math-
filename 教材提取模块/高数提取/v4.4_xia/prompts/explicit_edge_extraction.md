...
- `node_pool` 可能同时包含 Step 3E 已通过节点和待审节点。待审状态不是抽取禁令；只要原文证据明确，仍可输出候选边。后续脚本会把连接待审节点的边送入 review。
- 不补节点，不输出 supplement candidates。
- 不直接输出 `PREREQUISITE_OF`。
- 不输出普通 `APPLIES_TO`。
- 不输出条件判断规则边。条件判断由 Step 4B / 规则案例环节保存。
- 不要求你修正不确定方向。方向不确定时不要输出该边。
- 不确定就输出空数组。
- 空数组是正确答案。
...
EQUATIVE 默认是弱关系。只有原文明确并列列出同类对象时才输出。
...
不要把方向写成 `两行相同行列式为零性质 --HAS_PROPERTY--> n阶行列式`；应写成 `n阶行列式 --HAS_PROPERTY--> 两行相同行列式为零性质`。
...
```text
行列式计算问题 --USES--> 行列式按第i行展开公式
克莱姆法则 --USES--> 行列式按第j列展开公式
含参数线性方程组解的情况讨论方法 --USES--> 克莱姆法则
```

典型例题中，如果某题型或方法明显使用某公式/定理/方法，输出 `USES`。
...
如果原文只是把若干定理/公式“合起来称为”某个名称，应该交给节点别名、组合说明或人工归一化处理，不在 Step 4 中连边。

## Step 4A / Step 4B 分工

当前步骤是 Step 4A，只抽普通二元关系：

```text
SUPERIOR / PART_OF / HAS_PROPERTY / USES / GETS / DERIVES / EQUATIVE
```

条件判断、判别准则、充要条件、分情况结论留给 Step 4B / 规则案例环节处理：
...
这些信息应由 Step 4B / 规则案例环节保存，不在 Step 4A 中压成普通边。
...
例题关系必须使用具体工具节点。不要因为题目属于行列式计算，就把目标泛化成 `n阶行列式` 或 `n阶行列式的完全展开式`。例如原文说“按第1行展开 / 按这一列展开”时，只有 `node_pool` 中存在“行列式按行/列展开公式、定理、方法”这类具体节点才输出边；如果只有宽泛节点，输出空数组。
