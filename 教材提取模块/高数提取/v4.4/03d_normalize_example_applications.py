"""
v4.4 Step 3D: normalize example-derived application candidates.

Default behavior preserves the previous development-run behavior for
reproducibility. Use --clean-flow for a strict v4.4 run: Step 3D only performs
...
DEFAULT_CORE_NODES = DEFAULT_OUTPUT_DIR / "nodes_audited.jsonl"
...
def normalize_edges(
    ...
    for edge in app_edges:
        ...
        if source_layer == "core" and source_node.get("review_status") != "auto_accept":
            warnings.append("source_core_node_requires_review")
        if target_layer == "core" and target_node.get("review_status") != "auto_accept":
            warnings.append("target_core_node_requires_review")
        normalized_edge["validation_warnings"] = sorted(set(warnings))
        if any(warning in normalized_edge["validation_warnings"] for warning in ["source_core_node_requires_review", "target_core_node_requires_review"]):
            existing_reason = str(normalized_edge.get("review_reason") or "").strip()
            extra_reason = "应用层边连接了 Step 3E 待审核心节点，需随端点节点一起进入 Step 7 复核。"
            normalized_edge["review_reason"] = f"{existing_reason}；{extra_reason}" if existing_reason else extra_reason
        normalized_edge["edge_id"] = stable_id(
            f"{edge.get('textbook_id', '')}:norm-app-edge",
            [
...
def write_report(
    ...
    lines = [
        "# v4.4 Step 3D Normalized Example Application Report",
        "",
...
def main() -> None:
    ...
    audited_core_count = len({str(node.get("node_id") or "") for node in core_nodes if node.get("node_id")})
    ...
    with args.report.open("a", encoding="utf-8", newline="\n") as report_f:
        report_f.write(f"\n## Step 3E Core Pool\n- audited core nodes: {audited_core_count}\n- indexed core names: {len(core_index)}\n")
