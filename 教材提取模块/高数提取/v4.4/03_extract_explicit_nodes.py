"""
v4.4 Step 3: extract explicit KG node candidates from section summaries.

This step uses Step 2 summaries as lightweight reading context and validates
every candidate against the current leaf-section source text. It does not
extract relations.
"""
...
DEFAULT_REVIEW = DEFAULT_OUTPUT_DIR / "node_pre_audit_review_queue.jsonl"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extract v4.4 explicit node candidates.")
    ...
    parser.add_argument("--review", type=Path, default=DEFAULT_REVIEW, help="Pre-audit queue from schema validation; Step 3E writes the final node_review_queue.jsonl.")
...
def call_llm_with_json_fallback(
    ...
    try:
        ...
        fallback_prompt = (
            ...
            + "- Step 2 只提供小节梗概和关键词，不提供节点候选提示；仍需依据当前小节原文抽取。\n"
...
def detach_rule_cases_from_non_rule_owner(node: dict[str, Any], warnings: list[str]) -> list[str]:
    """Rule cases belong to theorem/formula/method owners; keep other valid nodes."""
    rule_cases = node.get("rule_cases", [])
    node_type = node.get("type", "")
    if not isinstance(rule_cases, list) or not rule_cases or node_type in {"Theorem", "Formula", "Method"}:
        return warnings

    node["dropped_rule_cases"] = [
        case if isinstance(case, dict) else {"raw": case}
        for case in rule_cases
    ]
    node["rule_cases"] = []
    node.setdefault("state_notes", [])
    if isinstance(node["state_notes"], list):
        node["state_notes"].append(
            "该节点是核心知识点，但 LLM 附带了条件判断；条件判断已从节点上剥离，避免节点因规则案例归属错误被拒绝。"
        )
    node["review_recommended"] = True
    if not node.get("review_reason"):
        node["review_reason"] = "条件判断不应直接挂在该类型节点上，已剥离 rule_cases 后保留节点候选。"

    filtered = [
        warning
        for warning in warnings
        if warning != "rule_cases_invalid_node_type" and not warning.startswith("rule_cases_invalid_node_type:")
    ]
    if "rule_cases_detached_from_non_rule_owner" not in filtered:
        filtered.append("rule_cases_detached_from_non_rule_owner")
    if "review_recommended" not in filtered:
        filtered.append("review_recommended")
    return filtered
...
def mock_nodes(section: dict[str, Any], summary: dict[str, Any]) -> dict[str, Any]:
    ...
    evidence = first_paragraph(section_text)
    for term in clean_list(summary.get("key_terms"))[:2]:
        if term and evidence:
            nodes.append({
                ...
                "description": "",
                "attributes": [],
                "state_notes": ["mock node from Step 2 key_terms; only for local pipeline validation."],
                "confidence": 0.50,
                "reason": "mock from lightweight section summary key_terms",
                "review_recommended": True,
                "review_reason": "mock 节点仅用于本地流程验证，不代表真实抽取结果。",
            })
    return {"nodes": nodes}
...
def write_report(
    ...
    lines = [
        "# v4.4 Step 3 Node Extraction Report",
        "",
...
def main() -> None:
    ...
    model = args.model or llm_config.get("node_model") or llm_config.get("default_model", "GPT-5.4")
    ...
    with (
        ...
        for summary in summaries:
            ...
            for index, raw_node in enumerate(raw_nodes, start=1):
                ...
                warnings = detach_rule_cases_from_non_rule_owner(node, warnings)
