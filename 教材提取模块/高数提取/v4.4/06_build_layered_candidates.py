...
explicit_core, example_application, review_pending, review_pending_merge, and
rejected_archive.
"""
...
DEFAULT_REVIEW_MERGE_CANDIDATES = DEFAULT_DIR / "step5_review_merge_candidates.jsonl"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--review-merge-candidates", type=Path, default=DEFAULT_REVIEW_MERGE_CANDIDATES)
...
def write_report(
    ...
    review_merge_candidates: list[dict[str, Any]],
    ...
    lines = [
        ...
        f"- review_pending_merge_candidates: {len(review_merge_candidates)}",
    ...
    for key, value in sorted(type_counts(review_merge_candidates, "node_type").items()):
        lines.append(f"- {key}: {value}")
    lines.extend(
        [
            "",
            ...
            "- merge candidate 只表示疑似同义/重复，不在 Step 6 执行合并。",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    review_merge_candidates_raw = read_jsonl(args.review_merge_candidates, required=False)
    ...
    review_merge_candidates = [with_step6(row, "review_pending_merge", "needs_decision") for row in review_merge_candidates_raw]
    ...
    write_jsonl(out_dir / "review_pending_merge_candidates.jsonl", review_merge_candidates)
