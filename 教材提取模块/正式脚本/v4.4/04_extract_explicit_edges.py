...
def maybe_rewrite_property_edge(edge: dict[str, Any]) -> list[str]:
    rewrites: list[str] = []
    edge_type = edge.get("type")
    if edge_type not in {"SUPERIOR", "HAS_PROPERTY"}:
        return rewrites
    source_name = str(edge.get("source_name") or "")
    source_type = str(edge.get("source_type") or "")
    target_type = str(edge.get("target_type") or "")
    looks_like_property = (
        PROPERTY_NAME_RE.search(source_name)
        or "性质" in source_name
    )
    if not looks_like_property:
        return rewrites
    if source_type in PROPERTY_TARGET_TYPES and target_type in PROPERTY_SOURCE_TYPES:
        ...
        edge["source_node_id"] = target_fields["target_node_id"]
        edge["source_name"] = target_fields["target_name"]
        edge["source_type"] = target_fields["target_type"]
        edge["source_review_status"] = target_fields["target_review_status"]
        edge["target_node_id"] = source_fields["source_node_id"]
        edge["target_name"] = source_fields["source_name"]
        edge["target_type"] = source_fields["source_type"]
        edge["target_review_status"] = source_fields["source_review_status"]
        edge["type"] = "HAS_PROPERTY"
        edge["description"] = f"{edge['source_name']} 具有性质：{edge['target_name']}。"
        edge["edge_id"] = stable_edge_id(edge)
        if original_type == "SUPERIOR":
            rewrites.append("superior_property_rewritten_to_has_property")
        else:
            rewrites.append("reversed_has_property_flipped")
    return rewrites
...
def validate_edge(edge: dict[str, Any], section_text: str, node_pool_ids: set[str]) -> list[str]:
    ...
    if edge.get("source_review_status") != "auto_accept" or edge.get("target_review_status") != "auto_accept":
        ...
        if not edge.get("review_reason"):
            edge["review_reason"] = "边连接了 review 节点，需要随节点一起审核。"
    ...
    if edge_type == "EQUATIVE":
        warnings.append("equative_weak_relation_requires_review")
        edge["review_recommended"] = True
        if not edge.get("review_reason"):
            edge["review_reason"] = "EQUATIVE 是弱关系/推荐关系，默认进入 review。"
    if edge_type == "DERIVES":
        warnings.append("derives_direction_requires_review")
        edge["review_recommended"] = True
        if not edge.get("review_reason"):
            edge["review_reason"] = "DERIVES 方向需要人工抽样确认：应为推导依据 -> 被推出结论。"
    return warnings
...
def process_raw_edge(
    ...
    rewrite_notes = maybe_rewrite_property_edge(edge)
    warnings = validate_edge(edge, section_text, node_pool_ids)
    if rewrite_notes:
        edge["edge_rewrites"] = rewrite_notes
        warnings.extend(rewrite_notes)
