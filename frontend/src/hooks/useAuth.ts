...
export function useAuth() {
  const [user, setUser] = useState<User>(() => {
    ...
    return { token, deviceId, userId: '', username: '', profile: null };
  });
    ...
    const initAuth = async () => {
      ...
      if (token) {
        try {
          ...
          setUser(prev => ({ ...prev, token, userId: profile.id, username: profile.username, profile }));
          return;
      ...
      try {
        ...
        setUser(prev => ({ ...prev, token: res.access_token, deviceId, userId: res.user_id, username: res.username }));
  ...
  const handleAuthSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === 'login') {
        ...
        setUser(prev => ({ ...prev, token: res.access_token, userId: res.user_id, username: res.username, profile }));
        // 迁移匿名期徽标到登录账号
        ...
        localStorage.setItem('auth_token', res.access_token);
        setUser(prev => ({ ...prev, token: res.access_token, userId: res.user_id, username: res.username }));
      }
