"""Chinese description to safe, bare LaTeX conversion with provider fallback."""
...
def _formula_example(description: str, latex: str) -> str:
    payload = json.dumps(
        {"latex": latex}, ensure_ascii=False, separators=(",", ":")
    )
...
def build_formula_completion_request(description: str) -> dict[str, object]:
    """Return the provider-independent portion of a conversion request."""
    return {
        "messages": [
...
class OpenAIFormulaProvider:
    ...
    async def convert(self, description: str, timeout: float) -> str:
        ...
        try:
            ...
            if self.llama_cpp:
                extra_body = dict(request.get("extra_body", {}))
            ...
            try:
                ...
                # Some OpenAI-compatible providers only implement json_object.
                response = await client.chat.completions.create(
...
def build_default_providers() -> list[FormulaProvider]:
    ...
    if config.FORMULA_LOCAL_API_BASE and config.FORMULA_LOCAL_MODEL:
        providers.append(OpenAIFormulaProvider(
            "local_qwen", config.FORMULA_LOCAL_API_KEY,
            config.FORMULA_LOCAL_API_BASE, config.FORMULA_LOCAL_MODEL,
            llama_cpp=True,
        ))
    if config.FORMULA_CLOUDFLARE_ACCOUNT_ID and config.FORMULA_CLOUDFLARE_API_TOKEN:
        providers.append(OpenAIFormulaProvider(
            "cloudflare_qwen", config.FORMULA_CLOUDFLARE_API_TOKEN,
            "https://api.cloudflare.com/client/v4/accounts/"
            f"{config.FORMULA_CLOUDFLARE_ACCOUNT_ID}/ai/v1",
            config.FORMULA_CLOUDFLARE_MODEL,
        ))
    if (
        config.FORMULA_EXISTING_API_BASE
        and config.FORMULA_EXISTING_API_KEY
        and config.FORMULA_EXISTING_MODEL
    ):
        providers.append(OpenAIFormulaProvider(
            "existing_model", config.FORMULA_EXISTING_API_KEY,
            config.FORMULA_EXISTING_API_BASE, config.FORMULA_EXISTING_MODEL,
        ))
    return providers
...
class FormulaConversionService:
    ...
    async def convert(
        self, description: str, preferred_display: DisplayPreference = "auto"
